...
class Xgboost4jCompatibilySpec extends WordSpec with Matchers with DoubleTolerant {
  ...
  private val model = XGBoost.train(trainingData, params, nbTrees, watches, null, null, null, 0, null)

  private val xgboostBinary = model.toByteArray
  private var predictor: Predictor = _

  "Predictor" should {

    "load model from a Xgboost4j binary model" in {
      predictor = Predictor(xgboostBinary)
    ...
    "predict like Xgboost4j" in {
      for (_<- 1 to nbTests) {
        val feats = (1 to nbFeatures).map(_ => rand.nextDouble).toArray
        val dmatrix = new DMatrix(feats.map(_.toFloat), 1, nbFeatures)
        predictor.predict(feats).head shouldEqual model.predict(dmatrix).head.head.toDouble
      }
    }

    "predictContrib like Xgboost4j" in {
      for (_<- 1 to nbTests) {
        val feats = (1 to nbFeatures).map(_ => rand.nextDouble).toArray
        val dmatrix = new DMatrix(feats.map(_.toFloat), 1, nbFeatures)
        predictor.predictContrib(feats) shouldEqual model.predictContrib(dmatrix, 0).head.map(_.toDouble)
      }
    }
  }
